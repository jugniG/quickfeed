      ...
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="max-w-[560px] mb-16">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <span className="text-[12px] font-medium uppercase tracking-[0.12em] text-neutral-400">The problem</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-14">
          {steps.map((s, i) => (
            <div key={i} className="relative p-6 rounded-2xl bg-white border border-neutral-200 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
              <div className="flex items-start justify-between mb-4">
                <span className="text-[26px] leading-none">{s.icon}</span>
                <span className="text-[11px] font-medium text-neutral-300 tracking-widest">{s.step}</span>
              </div>
              <h3 className="text-[14.5px] font-semibold text-[#0A0A0A] mb-2">{s.title}</h3>
              <p className="text-[13.5px] text-neutral-500 leading-[1.65] font-normal">{s.desc}</p>
              {i < 2 && (
                <div className="hidden md:block absolute top-1/2 -right-2.5 w-5 h-px bg-neutral-200" />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-neutral-200 rounded-2xl overflow-hidden border border-neutral-200">
          {stats.map((s, i) => (
            <div key={i} className="bg-white px-8 py-7">
              <div className="text-[2.2rem] font-bold tracking-[-0.04em] text-[#0A0A0A] mb-1.5">{s.value}</div>
